...
void IGAvailableCharacters::render(Game* g, IGManager * manager)
{
	...
	bool open = false;
	if(ImGui::Begin("chooseCharacter", &visible, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar))
	{
		...
		if(ImGui::BeginChild("chooseCharacterWrapper"))
		{
			...
			for (json::iterator it = characters.begin(); it != characters.end(); ++it) {
				...
				if(ImGui::BeginChild(childName.c_str(), sf::Vector2f(windowSize.x * 0.23f, 100), false, ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_AlwaysAutoResize))
				{
